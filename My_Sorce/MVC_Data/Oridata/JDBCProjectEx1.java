...
public class JDBCProjectEx1 extends JFrame implements ActionListener {
	...
	//component 객체 선언
		...
		public JDBCProjectEx1(){	//생서자 함수.	선언한것 초기화 작업.
			//component 등록
			panWest = new JPanel(new GridLayout(5, 0));
			p1 = new JPanel(new FlowLayout(FlowLayout.RIGHT));	//flow - 가운데를 중심으로 또는 왼쪽,오른쪽으로 붙일 수 있음.
			p1.add(new JLabel("번    호"));								//번호라는 이름을 넣음.
			p1.add(txtNo = new JTextField(12));							//12의 크기만큼 텍스트 필드 선언.
			panWest.add(p1);
			...
			//close
			this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);	//창 닫기 기능.
			//메인 창 출력
			setBounds(100, 100, 700, 300); //setSize(W,H);   pack();  , 100, 100 위치에서 크기 700 * 300 짜리 창 만들기.
			setVisible(true);		
		} //constuctor end
		
		@Override
		public void actionPerformed(ActionEvent e) {		//버튼 이벤트처리 ActionEvent- 이벤트와 관련된 모든 붓을 가져옴.
 			Object obj = e.getSource();			//obj 타입의 이유는 모든 타입을 다 받기 위해.
			if( obj == btnAdd ){
				if( cmd != ADD ){
					setText(ADD);  //user method setText - 사용자 메소드, 
					return;
				} //if in
		...
		private void setButton(int command) {
			...
			btnTotal.setEnabled(false);			//setEnable 활성화에 관한 것들.
